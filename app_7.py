...
def main():
    ...
    if (data_file_1 is not None):
        ...
        bank_raw_target_perc = bank_raw.y.value_counts(normalize = True).to_frame(name='proporção')*100
        bank_raw_target_perc = bank_raw_target_perc.sort_index()
        
        try:
            bank_target_perc = bank.y.value_counts(normalize = True).to_frame(name='proporção')*100
        ...
        if graph_type == 'Barras':
            ...
            bank_raw_target_perc.plot.pie(subplots=True, autopct='%.2f', ax=ax[0], legend=False)
            ax[0].set_title('Dados brutos',
                            fontweight ="bold")
            ax[0].legend(loc='upper center', bbox_to_anchor=(0.5, 0.13))
            
            bank_target_perc.plot.pie(subplots=True, autopct='%.2f', ax=ax[1], legend=False)
            ax[1].set_title('Dados filtrados',
                            fontweight ="bold")
            ax[1].legend(loc='upper center', bbox_to_anchor=(0.5, 0.13))
